...
st.set_page_config(page_title="Maize Yield Predictor", page_icon="🌽", layout="wide")
...
input_df = pd.DataFrame([{
    "Rainfall_mm": rainfall_mm, "Rainfall_LongRains_mm": rainfall_lr,
    "Rainfall_ShortRains_mm": rainfall_sr, "Temp_Avg_C": temp_avg,
    "Temp_Max_C": temp_max, "Temp_Min_C": temp_min,
    "Solar_Radiation": solar, "Humidity_Pct": humidity,
    "Soil_Wetness": soil_wetness, "NDVI_LongRains": ndvi_lr,
    "NDVI_ShortRains": ndvi_sr, "Variety_Code": variety_code,
    "Yield_Potential_Mid": yield_potential, "Maturity_Months": maturity_months
}])
...
with col_left:
    ...
    ax.plot(df_hist["Year"], df_hist["Yield_MT_Ha"], "o-", color="green", linewidth=2, label="Historical")
    ax.axhline(predicted, color="orange", linewidth=2, linestyle="--", label="Prediction: " + str(predicted))
    ax.fill_between([df_hist["Year"].min(), df_hist["Year"].max()], low, high, alpha=0.15, color="orange")
    ax.set_xlabel("Year")
...
with col_right:
    ...
    variety_info = {
        "H614D": (1, 7.0, 6.5), "DH04": (2, 8.0, 7.0),
        "DK8031": (3, 9.0, 6.5), "H6213": (4, 8.0, 7.0),
    }
    variety_preds = {}
    ...
    bar_colors = ["orange" if v == selected_name else "steelblue" for v in variety_preds.keys()]
    fig2, ax2 = plt.subplots(figsize=(8, 4))
    bars = ax2.bar(variety_preds.keys(), variety_preds.values(), color=bar_colors, alpha=0.85)
    ax2.bar_label(bars, fmt="%.3f", padding=3)
    ax2.set_ylabel("Predicted Yield (t/ha)")
    ax2.set_title("Same weather, different seed varieties")
...
st.dataframe(
    df_hist[["Year","Yield_MT_Ha","Rainfall_mm","Temp_Avg_C","NDVI_LongRains","Dominant_Variety","Source"]]
    .rename(columns={
        "Yield_MT_Ha": "Yield (t/ha)", "Rainfall_mm": "Rainfall (mm)",
        "Temp_Avg_C": "Avg Temp (C)", "NDVI_LongRains": "NDVI Long Rains",
